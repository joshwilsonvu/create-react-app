...
import { useMM, ModuleGuard } from 'mm-utils';
  ...
  const Compat = props => {
    ...
    useEffect(() => {
      ...
      if (l.requiresVersion && !semver.gt(l.requiresVersion, config.version)) {
        throw new Error(
          `Module ${Legacy.name} requires MM version ${l.requiresVersion}, running ${config.version}`
        );
      }
      l.setData(data);
      l.MM = MM;
      l.init();
      l.loaded(() => {}); // no longer required to call callback
